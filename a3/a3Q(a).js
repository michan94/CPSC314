...
function initMotions() {
    ...
    myboxMotion.addKeyFrame(new Keyframe('rest pose',2.0, [1,4,0]));
    myboxMotion.addKeyFrame(new Keyframe('rest pose',3.0, [0,5,0]));
    myboxMotion.addKeyFrame(new Keyframe('rest pose',4.0, [0,6,0]));
    myboxMotion.addKeyFrame(new Keyframe('rest pose',5.0, [0,5,0]));
    myboxMotion.addKeyFrame(new Keyframe('rest pose',6.0, [-1,4,0]));
    myboxMotion.addKeyFrame(new Keyframe('rest pose',7.0, [-1,1.9,0]));
    myboxMotion.addKeyFrame(new Keyframe('rest pose',8.0, [0,1.9,0]));
...
function handSetMatrices(avars) {
    ...
    link1.matrix.multiply(new THREE.Matrix4().makeRotationX(theta3));
    link1.matrix.multiply(new THREE.Matrix4().makeRotationX(theta4));   
    link1.matrix.multiply(new THREE.Matrix4().makeScale(5,1,2));   
    ...
    link2.matrix.multiply(new THREE.Matrix4().makeScale(5,1,1));    
    ...
    link3.matrix.multiply(new THREE.Matrix4().makeScale(5,1,1));    
    ...
    link4.matrix.multiply(new THREE.Matrix4().makeScale(5, 1, 1));    
    ...
    link5.matrix.multiply(new THREE.Matrix4().makeScale(5,1,1));    
...
function onResourcesLoaded(){
 ...
 //                             i.e., creates references to the geometry, and not full copies ]
    ...
    meshes["dragon2"].position.set(-5, 0, -4);
